...
def preprocess_dataset(dataset_id: int,
    ...
    # 注：原 distutils.file_util.copy_file 在 Python 3.12 已移除，改用 shutil + 仅在更新时复制
    import os
    import shutil
    ...
    for k in dataset:
        ...
        if not os.path.exists(dst) or os.path.getmtime(dataset[k]['label']) > os.path.getmtime(dst):
            shutil.copy2(dataset[k]['label'], dst)
