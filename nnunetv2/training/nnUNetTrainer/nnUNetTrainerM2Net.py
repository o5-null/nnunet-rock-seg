"""
M2Net 训练器 — Mamba-2 基础的 U2Net 风格嵌套架构
...
v2 迁移说明:
  ...
  - M2Net(in_ch, out_ch, deep_supervision) 构造简单，直接内联工厂逻辑
"""
...
from nnunetv2.nets.m2net import M2Net
import torch
...
class nnUNetTrainerM2Net(nnUNetTrainer_MedNeXtBase):
    """
    M2Net: Mamba-2 based U2Net-style nested architecture
    ...
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
        ...
        # nnUZoo 原版超参（对应 nnUZoo nnUNetTrainerM2Net.__init__）
    ...
    def _get_deep_supervision_scales(self):
        """
        M2Net 的侧输出 (d0-d6) 处于固定分辨率: d0(1×), d1(1×), d2(1/2), d3(1/4), d4(1/8), d5(1/16), d6(1/32)。
    ...
    def set_deep_supervision_enabled(self, enabled: bool):
        """
        M2Net 没有 .decoder 属性，deep supervision 已通过模型内部的 self.deep_supervision 管理。
        只需同步该标志即可。
        """
        if self.is_ddp:
    ...
    def build_network_architecture(plans_manager: PlansManager,
        ...
        print(f"M2Net built. Trainable parameters: {sum(p.numel() for p in model.parameters() if p.requires_grad):,}")
        return model
