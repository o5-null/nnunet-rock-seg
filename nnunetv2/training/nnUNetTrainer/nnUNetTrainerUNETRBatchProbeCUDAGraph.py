# -*- coding: utf-8 -*-
"""nnUNetTrainerUNETRBatchProbeCUDAGraph — nnUNetTrainerUNETR + 自动 batch 探测 + CUDA Graph 累积加速

组合 nnUNetTrainerBatchProbeCUDAGraph（探测 + graph 累积 replay）与 nnUNetTrainerUNETR。
...
MRO: [T, BatchProbeCUDAGraph, CUDAGraphMixin, BatchProbe, nnUNetTrainerUNETR, ...]
（2026-08-01 脚本验证菱形继承合法线性化）

用法:
  # 纯探测（写缓存后退出）
  python -m nnunetv2.run.run_training 2 2d 0 ^
      -tr nnUNetTrainerUNETRBatchProbeCUDAGraph -p nnUNetPlans_bs16 --probe only
  # 完整训练（缓存命中秒用 / 未命中先探测）
  python -m nnunetv2.run.run_training 2 2d 0 ^
      -tr nnUNetTrainerUNETRBatchProbeCUDAGraph -p nnUNetPlans_bs16
"""
...
class nnUNetTrainerUNETRBatchProbeCUDAGraph(nnUNetTrainerBatchProbeCUDAGraph, nnUNetTrainerUNETR):
    """nnUNetTrainerUNETR + 自动 batch 探测 + CUDA Graph 累积加速。"""
    pass
