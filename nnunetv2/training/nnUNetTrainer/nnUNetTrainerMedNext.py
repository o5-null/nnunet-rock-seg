...
class nnUNetTrainerMedNext(nnUNetTrainer_MedNeXtBase):
    """
    MedNeXt 基础训练器 —— 继承 MedNeXt 基类（含 TF32 加速 + 增强指标）
    无深度监督（通过 _ds_enabled = False 声明）。
    """
    _ds_enabled = False

    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        # 注意：新基类链不接收 unpack_dataset 参数，由 nnUNetTrainer 默认处理
...
class nnUNetTrainerV2_MedNeXt_L_kernel5(nnUNetTrainerMedNext):
    ...
    def build_network_architecture(plans_manager: PlansManager,
        ...
        model = create_mednextv1_large(num_input_channels, num_output_channels, 5, False)
...
class nnUNetTrainerV2_MedNeXt_B_kernel5(nnUNetTrainerMedNext):
    ...
    def build_network_architecture(plans_manager: PlansManager,
        ...
        model = create_mednextv1_base(num_input_channels, num_output_channels, 5, False)
...
class nnUNetTrainerV2_MedNeXt_M_kernel5(nnUNetTrainerMedNext):
    ...
    def build_network_architecture(plans_manager: PlansManager,
        ...
        model = create_mednextv1_medium(num_input_channels, num_output_channels, 5, False)

        return model

class nnUNetTrainerV2_MedNeXt_M_kernel5_100epochs(nnUNetTrainerV2_MedNeXt_M_kernel5):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
        self.num_epochs = 100
