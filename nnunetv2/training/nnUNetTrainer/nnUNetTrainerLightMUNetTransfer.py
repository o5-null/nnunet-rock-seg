"""
LightM-UNet 迁移学习训练器
多继承 nnUNetTrainerLightMUNet + TransferLearningBase
"""
...
class nnUNetTrainerLightMUNetTransfer(nnUNetTrainerLightMUNet, TransferLearningBase):
    """LightM-UNet 迁移学习训练器 — 渐进解冻 + Tversky Loss"""

    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        super().__init__(plans, configuration, fold, dataset_json, device=device)
        self._init_transfer_learning_params()
        self._print_transfer_config("LightMUNet Transfer Learning")
    ...
    def run_training(self):
        """运行训练"""
        self.print_to_log_file("Starting LightMUNet Transfer Learning training")
        start = time.time()
...
class nnUNetTrainerLightMUNetTransfer_100epochs(nnUNetTrainerLightMUNetTransfer):
    """100 轮版本的 LightM-UNet 迁移学习训练器"""
